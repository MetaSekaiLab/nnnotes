...
def decode(cat: Catalog, cue_sheet: str, out_dir: Path, key: int | None = None,
    ...
    for i, info in enumerate(infos, 1):
        ...
        stem = name if name not in seen else f"{name}_{i}"          # layered waveforms share a name
        if stem in stems:
            raise RuntimeError(f"{cue_sheet} stream {i}: file name {stem} already used")
        seen.add(name)
        stems.add(stem)
        plan.append((i, info, name, stem))
    ...
    def one(item) -> Path:
        ...
        if also is not None:
            ...
            if r.returncode != 0:
                raise RuntimeError(f"ffmpeg {also[0]} failed on {cue_sheet} stream {i}: {r.stderr[-2000:]}")
        if fmt == "wav":
    ...
    streams: list[dict] = []
    for (i, info, name, _), dst in zip(plan, dsts):
    ...
    if cache.enabled():
        ...
        meta_json = {"files": names, "result": {k: v.name for k, v in result.items()},
                     "cues": dumps(manifest, indent=1, ensure_ascii=True),
                     "streams": dumps(streams, indent=1, ensure_ascii=True)}
        DECODED.put(ck, cache.pack(json.dumps(meta_json).encode("utf-8"),
                                   [(out_dir / f).read_bytes() for f in names]))
    return result
